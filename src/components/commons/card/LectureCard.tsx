      ...
      <article className={isCard ? "py-[13px] px-[17px] border border-gray-300 rounded-[5px] bg-white" : ""}>
        <section className="flex justify-between items-center mb-3">
          <div className="flex items-center gap-1">
            <TagMain>{lecture.online ? "온라인" : "오프라인"}</TagMain>
            <TagLight>{lecture.category}</TagLight>
            <TagLight>{lecture.platform}</TagLight>
            <TagRate>{lecture.rating}</TagRate>
          </div>
          {isCard && (
            <Image src={isScraped ? bookmarkGrayOn : bookmarkGrayOff} alt="bookmark icon" width={24} height={24} />
          )}
        </section>
        <p className="mb-1 text-[14px] font-semibold tracking-[-0.28px] text-black">{lecture.title}</p>
        <p className="text-[12px] font-medium tracking-[-0.36px] text-gray-600">{lecture.instructor}</p>
      </article>
    </Link>
  );
}
