...
import { cancelApply, getApplication } from "@/lib/database/action/apply";
...
const StudyApplyingCard = (props: IStudyApplyingCardProps) => {
  ...
  const myApplicationStatus = (teamMembersId as TTeamMembersIdAddedMember)?.members?.[0].status;
  ...
  return (
    <div className="flex flex-col gap-4 px-5 py-6 bg-white border border-gray-300 rounded-lg">
      <Link href={`/study/${studyId}`} className="flex flex-col gap-4">
        {myApplicationStatus && <StudyApplyInfo status={myApplicationStatus} createAt={myApplicationCreatedAt} />}
        <StudyMeetingInfo
      ...
      <div className="flex gap-2">
        ...
        {myApplicationStatus !== "ACCEPTED" && (
          <form onSubmit={handleCancelApply} className="w-full">
